...
export function CatalogRootGrid({ items }: { items: CatalogRootCategory[] }) {
  return <div className="catalog-root-grid">{items.map((item) => <CatalogRootCard category={item} key={item.id} />)}</div>
}
