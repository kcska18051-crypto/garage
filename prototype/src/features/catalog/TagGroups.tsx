...
export function TagGroups({ groups, activeTag, onSelect }: { groups?: TagGroup[]; activeTag?: { group: string; value: string }; onSelect(group: string, value: string): void }) {
  ...
  return <section className="catalog-tags" aria-labelledby="catalog-tags-title"><div className="catalog-tags__heading"><h2 id="catalog-tags-title">Быстрый выбор по параметрам</h2></div><div className="catalog-tags__groups">{groups.map((group) => {
