      ...
      <div className="hero__stage">
        {slides.map((slide, index) => (
          <article className="hero__slide" key={slide.id} hidden={index !== active} aria-label={`Слайд ${index + 1} из ${slides.length}`}>
            <div className="hero__copy">{slide.deadline && <small>{slide.deadline}</small>}{index === 0 ? <h1>{slide.title}</h1> : <h2>{slide.title}</h2>}<p>{slide.text}</p><Link className="button button--dark" to={slide.href}>{slide.cta}<span aria-hidden="true">→</span></Link></div>
